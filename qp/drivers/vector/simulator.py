...
from qp.gates import Hadamard, Identity, X, Y, Z, S, Sd, T, Td, CNOT, SWAP
from .qubit import Qubits, Zero, One
from qp.measure import measure
...
def run_gate_array(
    ...
    for gate in gate_array:
        ...
        if isinstance(gate, gatearray.CNOT):
            expand_double_gate(
                simulator_gate, gate.ctrl, gate.target, num_qbits
            ) | qubits

        elif isinstance(gate, gatearray.SWAP):
            expand_double_gate(simulator_gate, gate.a, gate.b, num_qbits) | qubits
    ...
    for _ in range(num_measures):
        measurement = [measure(qubits, i, project=False) for i in range(num_qbits)]
